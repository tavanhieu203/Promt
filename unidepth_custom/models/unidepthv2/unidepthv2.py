...
from unidepth_custom.models.unidepthv2.decoder import Decoder
from unidepth_custom.utils.constants import (IMAGENET_DATASET_MEAN,
                                      IMAGENET_DATASET_STD)
from unidepth_custom.utils.distributed import is_main_process
from unidepth_custom.utils.geometric import (generate_rays,
                                      spherical_zbuffer_to_euclidean)
from unidepth_custom.utils.misc import (first_stack, last_stack, max_stack,
...
class UniDepthV2(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self.tokens = nn.ParameterList([nn.Parameter(torch.zeros(1, 1, 1024, device='cuda')) for _ in range(4)])
        # self.global_tokens = nn.ParameterList([nn.Parameter(torch.zeros(1, 1, 1024, device='cuda')) for _ in range(2)])
        # self.camera_tokens = nn.ParameterList([nn.Parameter(torch.zeros(1, 1, 1024, device='cuda')) for _ in range(4)])
    ...
    def forward(self, inputs, image_metas):
        ...
        # for i in range(4):
        #     tokens[i] += self.tokens[i]
        #     camera_tokens[i] += self.camera_tokens[i]
            
        # for i in range(2):
        #     global_tokens[i] += self.global_tokens[i]

        inputs["features"] = features            # 4  # torch.Size([1, 34, 45, 1024])
        inputs["tokens"] = tokens                # 4  # torch.Size([1, 1, 1024])
        inputs["global_tokens"] = global_tokens  # 2  # torch.Size([1, 1, 1024])
        inputs["camera_tokens"] = camera_tokens  # 4  # torch.Size([1, 1, 1024])
    ...
    def infer(self, rgbs: torch.Tensor, intrinsics=None):
        ...
        inputs = {}
        inputs["features"] = features           # len 4    # 1x42x56x1024
        inputs["tokens"] = tokens               # len 4    # 1x1x1024
        inputs["global_tokens"] = global_tokens # len 2    # 1x1x1024
        inputs["camera_tokens"] = camera_tokens # len 4    # 1x1x1024
        inputs["image"] = rgbs                  # No tuple # 1x3x588x784
    ...
    def build(self, config):
        mod = importlib.import_module("unidepth_custom.models.encoder")
